...
export default class UserLocationPageComponent extends Component {
	constructor(props) {
    super(props);
    	this.state = {
    		refreshing: false,
      ...
      return {
        title: navigation.getParam('user')+" at "+ navigation.getParam('place_name'),
        headerTitleStyle: {
    ...
    async _onRefresh() {
  	...
  	async refresh() {
	    ...
	    let response = await fetch(constants.link + '/api/Location?id=' + this.props.navigation.getParam("location") + '&username=' +this.props.navigation.getParam("user") , {
	      headers: {
